...
class GlobalMoranAlgorithm(HelpUrlMixin, QgsProcessingAlgorithm):
    INPUT = "INPUT"
    FIELD = "FIELD"
    ...
    def write_html_report(
        ...
        guidance_html = analyst_guidance_html(
            ...
            [
                "A significant result without a defensible neighborhood definition.",
                "A global result being used to claim exactly where the pattern occurs.",
                "Very sparse or very dense neighborhood support.",
            ],
            [
                "Incremental Spatial Autocorrelation for scale selection",
                "Local Moran's I for cluster/outlier locations",
                "Getis-Ord Gi* for hot/cold spot mapping",
            ],
            "Use this as a screening statistic. It can justify deeper local analysis, but it should not be the only evidence for a site-specific planning intervention.",
        )
