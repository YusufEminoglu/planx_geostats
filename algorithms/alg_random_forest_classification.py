# -*- coding: utf-8 -*-
"""Random Forest Classification Processing Algorithm."""
...
class RandomForestClassificationAlgorithm(HelpUrlMixin, QgsProcessingAlgorithm):
    INPUT = "INPUT"
    TARGET = "TARGET"
    FEATURES = "FEATURES"
    ...
    def icon(self):
        return algorithm_icon("random_forest_classification")

    def createInstance(self):
        return RandomForestClassificationAlgorithm()

    def shortHelpString(self) -> str:
        return (
            "Fits a Random Forest classifier to predict a categorical outcome "
            "(land-use class, zoning category, any field with a small number of "
            "distinct text or numeric codes) from an ensemble of decision trees "
            "voting by predicted-probability average. Classes are the distinct "
            "values seen in the target field among complete records - review the "
            "class list in the report before trusting rare-class results, since a "
            "class with only a handful of examples will be predicted unreliably "
            "regardless of forest size.\n\n"
            "Output: predicted class and prediction confidence (winning class "
            "probability) per complete record, plus an HTML report with accuracy, "
            "a full confusion matrix, and per-class precision/recall/F1.\n\n"
            "In-sample accuracy from this tool is optimistic for the same reason as "
            "Random Forest Regression's in-sample R2 - the forest can partly "
            "memorize training records. Cross-check with Spatial k-Fold "
            "Cross-Validation Evaluator before reporting an accuracy figure, "
            "especially with an imbalanced class distribution where a model that "
            "always predicts the majority class already scores high accuracy "
            "while being useless - look at per-class recall for the classes that "
            "actually matter for your planning question.\n\n"
            "Use SHAP Global Feature Importance to see which fields drive "
            "classification decisions, and Model Residual Spatial Autocorrelation "
            "Check on the confidence field to see whether misclassifications "
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterFeatureSink(self.OUTPUT, "Output Random Forest classification layer"))
        self.addParameter(
            QgsProcessingParameterFileDestination(
                self.HTML_REPORT, "Output Random Forest HTML report", fileFilter="HTML files (*.html)", optional=True,
            )
        )
        self.addOutput(QgsProcessingOutputHtml("HTML_REPORT_OUT", "Random Forest classification report"))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if not html_path:
            html_path = os.path.join(tempfile.gettempdir(), "planx_rf_classification_report.html")
        ...
        try:
            results = fit_random_forest_classifier(
                x, y, class_labels, n_estimators=n_estimators, max_depth=max_depth, min_samples_leaf=min_samples_leaf,
            )
        except ImportError as exc:
            raise QgsProcessingException(optional_dependency_error("Random Forest Classification", ["scikit-learn"], exc))

        feedback.pushInfo(f"Accuracy={results['accuracy']:.4f}, OOB score={results['oob_score']}")

        out_fields = source.fields()
        out_fields.append(QgsField("rf_class", QVariant.String, len=254))
        out_fields.append(QgsField("rf_conf", QVariant.Double, len=10, prec=4))
        ...
        for current, feature in enumerate(source.getFeatures()):
            ...
            if fid in result_map:
                ...
                out_feature.setAttribute("rf_class", class_labels[predicted_idx])
                out_feature.setAttribute("rf_conf", float(results["proba"][row_idx][predicted_idx]))
                out_feature.setAttribute("rf_used", 1)
            else:
                out_feature.setAttribute("rf_class", None)
                out_feature.setAttribute("rf_conf", None)
    ...
    def postProcessAlgorithm(self, context, feedback):
        ...
        apply_output_metadata(
            layer,
            "PlanX GeoStats Random Forest classification output",
            {
                "rf_class": "Random Forest predicted class label",
                "rf_conf": "Predicted-class probability (0-1); low values mean the forest was uncertain",
                "rf_used": "1 if the record had complete target/feature values and was used to fit and predict, 0 otherwise",
            },
            "random_forest_classification",
        )
        apply_renderer(layer, categorical_field_renderer(layer, layer.geometryType(), "rf_class"))
        return {}
    ...
    def _write_html(self, path, target_field, feature_fields, class_labels, results, skipped):
        ...
        guidance = analyst_guidance_html(
            ...
            "captures non-linear class boundaries between explanatory fields.",
            [
                "Every class has enough examples (tens, not a handful) to learn from.",
                "OOB score is close to reported accuracy.",
                "Per-class recall is checked, not just overall accuracy, when classes are imbalanced.",
            ],
            [
                "A majority class dominates and per-class recall for minority classes is near zero.",
                "OOB score far below in-sample accuracy.",
                "Confusion concentrated between two adjacent, easily-confused classes.",
            ],
            [
                "Spatial k-Fold Cross-Validation Evaluator - a stronger out-of-sample estimate.",
                "SHAP Global Feature Importance - which fields drive the classification.",
                "Model Residual Spatial Autocorrelation Check on rf_conf - do errors cluster spatially?",
            ],
            "Treat rf_conf as a confidence surface you can map: low-confidence "
            "areas are where field verification or additional explanatory data "
            "would improve the model most.",
        )
        content = f"""<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Random Forest Classification Report</title>
<style>
...
<div class="summary">
Accuracy = {results['accuracy']:.6f} | OOB score = {oob_text}<br>
