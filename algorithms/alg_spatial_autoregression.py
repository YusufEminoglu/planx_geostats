...
class SpatialAutoregressionAlgorithm(HelpUrlMixin, QgsProcessingAlgorithm):
    INPUT = "INPUT"
    DEP_VAR = "DEP_VAR"
    ...
    def shortHelpString(self) -> str:
        return (
            "Fits a global Spatial Lag (Spatial Autoregressive) model via "
            "PySAL's spreg.ML_Lag: y = rho * W*y + X*beta + error, where the "
            "dependent variable at each location depends partly on a "
            "weighted average of the dependent variable at neighboring "
            "locations. Use this over plain OLS when OLS residuals show "
            "significant spatial autocorrelation AND the planning question "
            "suggests a genuine substantive spillover process - e.g. "
            "property values that respond to neighboring property values, "
            "not just to shared local characteristics.\n\n"
            "Output: fitted values, residuals, standardized residuals, the "
            "spatial lag values (W*y), neighbor counts, and a "
            "complete-record audit flag per feature; the HTML report covers "
            "coefficient estimates, the spatial autoregressive parameter "
            "rho, model fit, weight-matrix support, and analyst caveats.\n\n"
            "Rho's sign and significance are the headline result: a "
            "significant positive rho confirms genuine spatial spillover in "
            "the dependent variable itself, beyond what X explains. But "
            "Spatial Lag and Spatial Error (this group) test for different "
            "mechanisms and are not interchangeable - if the true process is "
            "error-driven (omitted spatially-structured variables) rather "
            "than a real lag effect, ML_Lag can still fit and even look "
            "plausible while misattributing the source of the dependence. "
            "Run both and compare with Model Comparison Matrix. Requires "
            "libpysal and spreg - run GeoStats Library Status first if "
            "unsure whether they're installed."
        )
