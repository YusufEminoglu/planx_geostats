...
class SpatialErrorRegressionAlgorithm(HelpUrlMixin, QgsProcessingAlgorithm):
    ...
    def shortHelpString(self) -> str:
        return (
            "Fits a global Spatial Error Model via PySAL's spreg.ML_Error: "
            "the dependent variable is modeled as a standard linear function "
            "of X, but the error term itself is spatially autocorrelated "
            "(lambda * W*u + epsilon) rather than the dependent variable. "
            "Use this when OLS or GLR residuals remain spatially "
            "autocorrelated but the mechanism looks like nuisance dependence "
            "- omitted spatially-structured variables, shared administrative "
            "boundaries, or spatially correlated measurement error - rather "
            "than a genuine spillover in the outcome itself.\n\n"
            "Output: fitted values, residuals, standardized residuals, "
            "neighbor counts, and complete-record audit flags per feature; "
            "the HTML report covers lambda (the spatial error parameter), "
            "coefficient estimates, model fit, input diagnostics, residual "
            "Moran's I, caveats, and recommended next actions.\n\n"
            "Unlike Spatial Lag's rho, SEM's lambda does not change the "
            "interpretation of the X coefficients - they remain directly "
            "comparable to the OLS coefficients, just with corrected "
            "standard errors. This is SEM's main practical advantage when "
            "the analyst mainly cares about getting valid inference on X, "
            "not about modeling a spillover mechanism. If residual Moran's I "
            "remains significant even after fitting SEM, the spatial "
            "structure may not be pure error dependence - reconsider Spatial "
            "Lag or check for a missing explanatory variable with real "
            "spatial structure. Requires libpysal and spreg in the active "
