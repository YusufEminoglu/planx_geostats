...
- **37 new algorithms, taking the plugin from 44 to 81**, across three new groups plus one existing group:
  ...
  - **07 | Network Centrality and Space Syntax (5 tools)**: Network Connectivity Diagnostics, Betweenness/Closeness/Straightness Centrality, and Network Reach — classical graph-theory (Freeman) centrality on a `networkx` graph recovered from line-layer geometry via endpoint snapping, explicitly documented as distinct from angular-segment space-syntax analysis (NACH/NAIN, Depthmap-style choice/integration).
  - **08 | Accessibility (3 tools)**: Two-Step Floating Catchment Area (2SFCA), Gravity-Based Accessibility Index (Gaussian/Exponential/Power decay), and Nearest-Facility Coverage Gap — straight-line distance between demand and supply layers.
...
- New optional dependencies (`xgboost`, `lightgbm`, `shap`, `networkx`) install through the existing Setup and Diagnostics > Install / Update GeoStats Libraries workflow, alongside `numba`/`libpysal`/`esda`/`spreg`/`mgwr`/`scikit-learn`.
- Three new synthetic QA/demo GeoPackages (`planx_geostats_network_qa`, `planx_geostats_classification_qa`, `planx_geostats_accessibility_qa`) fill gaps the bundled Izmir FUR sample cannot cover on its own — it has no line-network geometry (only pre-computed zone-level network summary fields) and no categorical field. Sample Dataset Guide and Workflow Advisor updated to load and recommend all five bundled datasets.
- Reference manual expanded with a full Theoretical Background / Mathematical Formulation / Parameters / Output / Interpretation Guide / Literature entry for every new tool (44 to 81 manual entries), each with real academic citations. Manual hero stats, sidebar navigation, and workflow diagram updated for 9 analytical groups.
